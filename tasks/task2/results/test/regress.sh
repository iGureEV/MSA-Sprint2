...
if [[ -z "${DB_HOST:-}" ]] || [[ -z "${DB_PORT:-}" ]]; then
  echo "❌ Не заданы переменные окружения DB_HOST или DB_PORT"
  exit 1
fi
...
# Проверка соединения с БД бронирования
echo "🧪 Проверка подключения к БД..."
if [[ -z "${DB_HOST_BOOKING:-}" ]] || [[ -z "${DB_PORT_BOOKING:-}" ]]; then
  echo "❌ Не заданы переменные окружения DB_HOST_BOOKING или DB_PORT_BOOKING"
  exit 1
fi
...
# Загрузка фикстур бронирования
echo "🧪 Загрузка фикстур микросервиса бронирования..."
PGPASSWORD="${DB_PASSWORD_BOOKING}" psql -h "${DB_HOST_BOOKING}" -p "${DB_PORT_BOOKING}" -U "${DB_USER_BOOKING}" "${DB_NAME_BOOKING}" < init-booking-service.sql
...
echo ""
echo "Тесты микросервисной реализации бронирования..."
echo "🧪 Тестирование микросервисной реализации с параметром ms=true"

# 1. Получение всех бронирований через микросервис
curl -sSf "${BASE}/api/bookings?ms=true" | grep -q 'test-user-2' && pass "Все бронирования получены (микросервис)" || fail "Бронирования не получены (микросервис)"

# 2. Получение бронирований пользователя через микросервис
curl -sSf "${BASE}/api/bookings?userId=test-user-2&ms=true" | grep -q 'test-user-2' && pass "Бронирования test-user-2 найдены (микросервис)" || fail "Нет бронирований test-user-2 (микросервис)"

# 3. Успешное бронирование отеля без промо через микросервис
curl -sSf -X POST "${BASE}/api/bookings?userId=test-user-3&hotelId=test-hotel-1&ms=true" | grep -q 'test-hotel-1' && pass "Бронирование прошло (без промо, микросервис)" || fail "Бронирование (без промо) не прошло (микросервис)"

# 4. Успешное бронирование с промо через микросервис
curl -sSf -X POST "${BASE}/api/bookings?userId=test-user-2&hotelId=test-hotel-1&promoCode=TESTCODE1&ms=true" | grep -q 'TESTCODE1' && pass "Бронирование с промо прошло (микросервис)" || fail "Бронирование с промо не прошло (микросервис)"

# 5. Ошибка — неактивный пользователь через микросервис
code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE}/api/bookings?userId=test-user-0&hotelId=test-hotel-1&ms=true")
if [[ "$code" == "500" ]]; then
  pass "Отклонено: неактивный пользователь (микросервис)"
else
  fail "Ошибка: сервер принял бронирование от неактивного пользователя (код $code) (микросервис)"
fi

# 6. Ошибка — отель не доверенный через микросервис
curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE}/api/bookings?userId=test-user-2&hotelId=test-hotel-3&ms=true" | grep -q '500' \
  && pass "Отклонено: недоверенный отель (микросервис)" \
  || fail "Ошибка: сервер принял бронирование от недоверенного отеля (микросервис)"

# 7. Ошибка — отель полностью забронирован через микросервис
curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE}/api/bookings?userId=test-user-2&hotelId=test-hotel-2&ms=true" | grep -q '500' \
  && pass "Отклонено: отель полностью забронирован (микросервис)" \
  || fail "Ошибка: сервер принял бронирование в полностью занятом отеле (микросервис)"
